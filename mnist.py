...
class MNISTModel(object):
    ...
    def _build_model(self):
        self.X = tf.placeholder(tf.uint8, [None, 28, 28, 3])  # 输入
        self.y = tf.placeholder(tf.float32, [None, 10])  # 标签
        self.domain = tf.placeholder(tf.float32, [None, 2])  # 域，为什么纬度是2 ？？？
        self.l = tf.placeholder(tf.float32, [])  # 反转
        self.train = tf.placeholder(tf.bool, [])  #

        X_input = (tf.cast(self.X, tf.float32) - pixel_mean) / 255.  # normalize X
        ...
        for i in range(num_steps):
            ...
            if training_mode == 'dann':
                ...
                if verbose and i % 100 == 0:
                    print('step {}, loss: {}  d_acc: {}  p_acc: {}  p: {}  l: {}  lr: {}'.format(
                        i, batch_loss, d_acc, p_acc, p, l, lr))

            elif training_mode == 'source':
                X, y = next(gen_source_only_batch)
                _, batch_loss, p_acc = sess.run([regular_train_op, pred_loss, pred_loss],
                                         feed_dict={model.X: X, model.y: y, model.train: False,
                                                    model.l: l, learning_rate: lr})
                print('step {}, loss: {}  p_acc: {}  p: {}  l: {}  lr: {}'.format(
                    i, batch_loss, p_acc, p, l, lr))
